      ...
      >
        ...
        <el-table-column prop="user_id" label="用户ID">
          <template #default="{row}">
            <el-tooltip class="item" effect="dark" placement="top" :content="row.user_id">
              <el-text disabled truncated style="width: 80%">{{row.user_id}}</el-text>
            </el-tooltip>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="disabled" label="账号已禁用" >
          <template #default="{row}">
            <el-switch v-model="row.disabled" active-text="是" inactive-text="否" active-color="#13ce66" inactive-color="#ff4949" disabled/>
          </template>
        </el-table-column>
